...
const signedMoney = (v, symbol) => `${v > 0 ? '+' : v < 0 ? '-' : ''}${symbol}${Math.abs(v).toFixed(2)}`;
        ...
        <View style={styles.insuranceBox}>
          ...
          >
            ...
            <Text style={styles.insuranceText}>
              Took insurance{betValid ? ` (${currencySymbol}${(bet / 2).toFixed(2)})` : ''}
            </Text>
          </TouchableOpacity>
          {insurance && (
            <>
        ...
        <View style={[styles.card, SHADOWS.card]}>
          ...
          {isSurrender && (
            <Text style={styles.helperText}>
              Surrender gives back half your bet{betValid ? ` (−${currencySymbol}${(bet / 2).toFixed(2)})` : ''}.
            </Text>
          )}
